...
input is the net dollar creation and redemption of exchange-traded fund (ETF) shares,
which each fund reports monthly to the US Securities and Exchange Commission (SEC) on
Form N-PORT. We normalize these flows by fund size, aggregate to sectors, and compare
each sector both to the market as a whole and to its own history. The result is a
monthly sector-flow panel (one observation per sector per month) covering 2019 to 2026 whose
...
A fund's assets under management (AUM, the market value of everything the fund holds on
behalf of its investors) move for two reasons. The market reprices the holdings the fund
already owns, and investors add or withdraw money. Only the second is a flow. Over one
period the two combine as
\begin{equation}
\label{eq:aum_identity}
A_t = A_{t-1}\,(1 + r_t) + F_t,
\end{equation}
where $A_t$ is assets under management at the end of period $t$, $r_t$ is the fund's
total return over the period, and $F_t$ is the net dollar flow. Total return is the
fractional gain on the holdings counting both price changes and the income they pay out,
such as dividends. The first term is therefore what the previous period's money grew to
on its own. Anything beyond it is new investor money.
...
An exchange-traded fund trades on an exchange like a stock, but unlike a stock its share
count is not fixed: the fund issues new shares when demand arrives and cancels them when
demand leaves, each exchanged at approximately net asset value (NAV), the per-share
market value of the fund's holdings. The change in shares outstanding therefore reveals
the flow,
\begin{equation}
...
N-PORT, the portfolio report the SEC requires of every US registered fund, supplies the
flow directly instead. Funds file it monthly, and each filing reports, for each of the
three months in its quarter, the total dollar value of share sales, redemptions, and
dividend reinvestment (Item B.8 of the form), the monthly total return (Item B.7), and
net assets at the period end (Item B.1). The net flow is
\begin{equation}
\label{eq:flow_reported}
F_t = \text{sales}_t - \text{redemptions}_t,
\end{equation}
with reinvestment excluded: a reinvested dividend buys new shares using cash the fund
itself paid out, so no outside money has moved. This is the realized
...
it counts dividends as if they stayed invested, but the dividend cash is actually paid
out to investors and leaves the fund, so an unbroken roll overstates assets in
proportion to dividend yield. Re-anchoring at every report bounds that bias to
...
score -- a z-score, distance from the recent norm in units of typical variation,
\begin{equation}
\label{eq:zscore}
...
typically traces these quadrants clockwise, and the path is the rotation. The
construction mirrors the relative rotation graph (RRG) practitioners use to chart
price-based relative strength; here the underlying signal is flow.
...
coverage from late 2019. The universe is the eleven Select Sector SPDR funds (XLK, XLE,
XLF, and so on), the ETF family that divides the S\&P 500 into the eleven sectors of the
Global Industry Classification Standard (GICS), one fund per sector; together they tile
the US large-cap equity market. Each sector currently holds one fund, so
