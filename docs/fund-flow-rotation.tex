...
unusual relative to its own norm. For a comparable picture we standardize relative flow
against its own recent history, exactly as in \eqref{eq:zscore},
\begin{equation}
\label{eq:rs_ratio}
\mathrm{RS}_{c,t} = \frac{\mathrm{rel}_{c,t} - \mu_L(\mathrm{rel}_c)}{\sigma_L(\mathrm{rel}_c)},
\end{equation}
...
A category typically traces these quadrants clockwise, and the path is the rotation.
