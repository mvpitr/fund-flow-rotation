...
Rotation is movement of capital between categories over time. Four statistics describe it.
...
across them. Standardizing against each category's own trailing history of length $L$
gives a comparable score,
\begin{equation}
\label{eq:zscore}
z_{c,t} = \frac{g_{c,t} - \mu_{L}(g_c)}{\sigma_{L}(g_c)},
\end{equation}
with $\mu_L$ and $\sigma_L$ the rolling mean and standard deviation. A value $z_{c,t}=2$
means the category is drawing unusually strong inflows relative to its own norm.
...
\end{equation}
Plotting categories by relative flow \eqref{eq:relative_flow} against momentum
\eqref{eq:momentum} places each in one of four quadrants: leading (strong and rising),
weakening (strong and falling), lagging (weak and falling), and improving (weak and
rising). A category typically traces these clockwise, and the path is the rotation.

\section{Validation}
\label{sec:validation}

The flows are reported rather than estimated, but the reconstructed quantities admit two
independent checks. Both run over the eleven-sector panel of Section~\ref{sec:data},
